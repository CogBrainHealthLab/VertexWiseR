...
The analysis will use surface data already extracted in R from a preprocessed subjects directory, which we make available so you do not need to preprocess a sample yourself.

To obtain it, we had extracted cortical thickness (CT) data from a Hippunfold preprocessing directory of  the [Fink dataset](https://openneuro.org/datasets/ds003799/versions/2.0.0) [@fink_two-week_2021]. Here is the command line which was used to extract it:

```{r}
#HIPvextract(sdirpath = hippunfold_SUBJECTS_DIR, filename = "FINK_Tv", measure = "thickness", subj_ID = T)
```

In this demo, the hippocampal surface data can be loaded straight from the online repository and then smoothed:
