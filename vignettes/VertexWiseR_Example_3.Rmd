...
Since [SubCortexMesh v.1.1.0](https://subcortexmesh.readthedocs.io/en/latest/atlases.html), anatomical sub-parcellations are also available for surfaces in fsaverage space, and give more specific location in regions' nuclei or subfields. VertexWiseR can use them with the argument 'atlas=2' (the caudate nuclei do not have subparcellations, so we show it for the thalami):
...
model=RFT_vertex_analysis(model = as.factor(beh_data$group),
                          contrast = as.factor(beh_data$group),
                          surf_data = get(surf_data),
                          atlas=2) #anatomical atlas, applicable for fsaverage SCM data
assign(paste0('model_',surf_data),model)
}
```
```{r}
...
We can plot the model results together in one summary plot by binding the maps:
...
knitr::include_graphics('allaseg_plotsurf3d.png')
```
